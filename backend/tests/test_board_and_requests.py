...
def test_my_tasks_lists_campfires_with_progress(client, make_token, seeded, db_session):
    """My Tasks shows happening fires and percent-complete progress bars."""
    from app.db.seed import SEED_EVENT_IDS
    from app.models.event_summary import Event

    rally = db_session.get(Event, SEED_EVENT_IDS["fall_rally_2026"])
    ...
    other = seeded["community_head"]

    make_task(
        client,
        head,
        title="My flyer",
        eventId=str(rally.id),
        assigneeUserId=str(member.id),
    )
    make_task(
        client,
        head,
        title="Head checklist",
        eventId=str(rally.id),
        assigneeUserId=str(other.id),
    )
    done = make_task(
        client,
        head,
    ...
    mine = client.get(
        "/tasks/mine", headers=auth_header(make_token, member.id)
    )
    assert mine.status_code == 200
    body = mine.json()
    assert body["openTaskCount"] >= 1
    campfire = next(
        c for c in body["campfires"] if c["event"]["id"] == str(rally.id)
    )
    assert campfire["tone"] == "now"
    assert campfire["progress"]["total"] == 3
    assert campfire["progress"]["done"] == 1
    assert campfire["progress"]["percentComplete"] == 33
    assert [t["title"] for t in campfire["myTasks"]] == ["My flyer"]
    by_name = {a["name"]: a for a in campfire["assignees"]}
    me_name = member.full_name or member.email
    other_name = other.full_name or other.email
    assert by_name[me_name]["isMe"] is True
    assert by_name[me_name]["percentComplete"] == 0
    assert by_name[other_name]["isMe"] is False
    assert by_name[other_name]["percentComplete"] == 50


def test_my_tasks_requires_auth(client):
    assert client.get("/tasks/mine").status_code == 401
